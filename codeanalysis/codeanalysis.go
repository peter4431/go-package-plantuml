...
func findGoPackageNameInDirPath(dirpath string) string {
	...
	if e != nil {
		log.Errorf("读取目录%s文件列表失败,%s", dirpath, e)
		return ""
	}
...
func (this *analysisTool) analysis(config Config) {
	...
	_ = filepath.Walk(config.CodeDir, dir_walk_once)
	...
	_ = filepath.Walk(config.CodeDir, dir_walk_twice)
...
func (this *analysisTool) OutputToFile(logfile string) {
	...
	uml := this.UML()
	err := ioutil.WriteFile(logfile, []byte(uml), 0666)
	if err != nil {
		log.Errorf("writeFile:%s err:%v", logfile, err)
	}
	log.Infof("数据已保存到%s\n", logfile)
}
